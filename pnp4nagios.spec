...
BuildRequires:  autoconf, automake, libtool, rrdtool, rrdtool-perl, perl(Time::HiRes)
...
%configure --prefix='' \
           ...
           --datadir=%{_datadir}/naemon/html/%{name} \
           --datarootdir=%{_datadir}/naemon/html/%{name} \
           %if 0%{with_selinux} 
           --enable-selinux=yes \
           %else
           --with-selinux=no \
           ...
           --with-kohana_system=%{_datadir}/naemon/html/%{name}/kohana/system \
...
%attr(755,naemon,naemon) %{_localstatedir}/lib/%{name}
%dir %attr(755,naemon,naemon) %{_localstatedir}/log/%{name}
%dir %attr(755,naemon,naemon) %{_localstatedir}/log/%{name}/stats
%dir %attr(755,naemon,naemon) %{_localstatedir}/log/%{name}/lock
%dir %attr(755,apache,apache) %{_localstatedir}/log/%{name}/kohana
%dir %attr(755,naemon,naemon) %{_localstatedir}/spool/%{name}
%{_datadir}/naemon/html/%{name}/
%{_mandir}/man8/*
...
then
        perl %{_sysconfdir}/pnp4nagios/config_tools/TemplateMod.pl -i %{_sysconfdir}/naemon/objects/templates.cfg -o config_samples/naemon/objects/templates.cfg
fi
if [ -e %{_sysconfdir}/naemon/naemon.cfg ] ;
then
    nagver=`naemon --version | gawk '/^Naemon /'  | gawk -v RS=' ' '/^[0-9]/'`
    perl %{_sysconfdir}/pnp4nagios/config_tools/NagiosCfgMod.pl -i %{_sysconfdir}/naemon/naemon.cfg -m sync -o config_samples/nagios/nagios-sync.cfg -n $nagver
    perl %{_sysconfdir}/pnp4nagios/config_tools/NagiosCfgMod.pl -i %{_sysconfdir}/naemon/naemon.cfg -m bulk -o config_samples/nagios/nagios-bulk.cfg -n $nagver
    perl %{_sysconfdir}/pnp4nagios/config_tools/NagiosCfgMod.pl -i %{_sysconfdir}/naemon/naemon.cfg -m npcd -o config_samples/nagios/nagios-npcd.cfg -n $nagver
fi        
